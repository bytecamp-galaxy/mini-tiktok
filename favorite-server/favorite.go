...
import (
	"context"
	"github.com/bytecamp-galaxy/mini-tiktok/favorite-server/kitex_gen/favorite"
	"github.com/bytecamp-galaxy/mini-tiktok/favorite-server/kitex_gen/feed"
	"github.com/bytecamp-galaxy/mini-tiktok/favorite-server/kitex_gen/user"
...
func (s *FavoriteServiceImpl) _FavoriteList(ctx context.Context, req *favorite.FavoriteListRequest) ([]*feed.Video, error) {
	Favoritevideos := []model.Video{}
	videos, err := FavoriteVideos(ctx, Favoritevideos, &req.UserId)
	return videos, err
}

func FavoriteVideos(ctx context.Context, vs []model.Video, uid *int64) ([]*feed.Video, error) {
...
func Videos(ctx context.Context, vs []*model.Video, fromID *int64) ([]*feed.Video, error) {
	respVideos := make([]*feed.Video, len(vs))
