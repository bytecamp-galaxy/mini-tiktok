...
import (
	"context"
	"fmt"
	"github.com/bytecamp-galaxy/mini-tiktok/cmd/relation/handler"
...
func main() {
	v := conf.Init()
	// init log
	log.SetOutput(v.GetString("relation-server.log-path"))
	log.InitKLogger()
	...
	serverAddr := fmt.Sprintf("%s:%d", v.GetString("relation-server.host"), v.GetInt("relation-server.port"))
	...
	p := provider.NewOpenTelemetryProvider(
		provider.WithServiceName(v.GetString("relation-server.name")),
	...
	svr := relationservice.NewServer(
		...
		server.WithServerBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: v.GetString("relation-server.name")}),
